...
HeyMacFrame::HeyMacFrame(uint8_t * buf, uint8_t sz)
{
    ...
    if(sz == 0)
        ...
        _buf[FRM_IDX_PID] = HM_PIDFLD_INVALID;
        _buf[FRM_IDX_FCTL] = 0;
        }
    _payld_offset = 0;
    _payld_sz = 0;
...
uint8_t HeyMacFrame::get_sz(void)
{
    ...
    {
        if (_buf[FRM_IDX_FCTL] & FCTL_BIT_M)
        {
            mhop_sz = (_buf[FRM_IDX_FCTL] & FCTL_BIT_L) ? 1 + 8 : 1 + 2;
        }
...
{
    _buf[FRM_IDX_PID] = pidfld;
}

void HeyMacFrame::set_net_id(uint16_t net_id)
{
    /* MSB first (big endian) */
    _buf[FRM_IDX_NETID] = net_id >> 8;
    _buf[FRM_IDX_NETID + 1] = net_id & 0xFF;

    _buf[FRM_IDX_FCTL] |= FCTL_BIT_N;
}
...
void HeyMacFrame::set_dst_addr(uint16_t dst_addr)
{
    ...
    if (_buf[FRM_IDX_FCTL] & FCTL_BIT_N)
        offset += 2;
    ...
    _buf[FRM_IDX_FCTL] |= FCTL_BIT_D;
    _buf[FRM_IDX_FCTL] &= ~FCTL_BIT_L;
}
...
{
//    assert((_buf[FRM_IDX_FCTL] & FCTL_BIT_L) == 0);
    ...
    if (_buf[FRM_IDX_FCTL] & FCTL_BIT_D)
    {
        // TODO: shouldn't support Long addr here since this method sets the Short addr
        offset += (_buf[FRM_IDX_FCTL] & FCTL_BIT_L) ? 8 : 2;
    }
    if (_buf[FRM_IDX_FCTL] & FCTL_BIT_I)
    {
...
bool HeyMacFrame::set_payld(uint8_t * payld, uint8_t sz)
{
    ...
    if (_buf[FRM_IDX_FCTL] & FCTL_BIT_S)
    {
        offset += (_buf[FRM_IDX_FCTL] & FCTL_BIT_L) ? 8 : 2;
    }
...
bool HeyMacFrame::set_mhop(uint8_t hops, uint16_t tx_addr)
{
    ...
    if (((_buf[FRM_IDX_FCTL] & FCTL_BIT_M) == 0)
     &&((_buf[FRM_IDX_FCTL] & FCTL_BIT_L) == 0))
    {
